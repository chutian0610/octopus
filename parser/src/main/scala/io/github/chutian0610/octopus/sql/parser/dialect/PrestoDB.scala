...
import io.github.chutian0610.octopus.common.util.Engine.PRESTO_DB
...
class PrestoDB extends SqlDialect(engine = PRESTO_DB) {
  ...
  override def matchKeyWord(s: String): Option[KEYWORD] = {
    KEYWORDS.withNameInsensitiveOption(s).filter(x=>PrestoDB.KeyWordSet.contains(x))
  }


  override def matchReservedWords(s: String): Option[String] = {
    KEYWORDS.withNameInsensitiveOption(s).filter(x => PrestoDB.ReservedKeyWords.contains(x)).map(x => x.entryName)
  }
}
